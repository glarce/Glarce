...
function linkMediaFolder() {
  ...
  for (let i = 0; i < dirs.length; i++) {
    console.log(dirs[i])
    console.log(normalize(`${__dirname}/../../../media/${dirs[i]}`))
    console.log(normalize(`${__dirname}/../public/${dirs[i]}`))
    
    symlinkDir(normalize(`${__dirname}/../../../media/${dirs[i]}`), normalize(`${__dirname}/../public/${dirs[i]}`))
      .then(() => {
        return symlinkDir(normalize(`${__dirname}/../../../media/${dirs[i]}`), normalize(`${__dirname}/../public/${dirs[i]}`))
      })
